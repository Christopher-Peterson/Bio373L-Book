...
```{r a6_sac_expt1}
bind_rows( # calculate SAC for high & low disturbance, then plot together
  wide_ant_data %>% 
    filter(Disturbance == "high") %>% 
    get_sac() %>% 
    mutate(Disturbance = "high"), # Add disturbance column back to sac results
  wide_ant_data %>% 
    filter(Disturbance == "low") %>% 
    get_sac() %>% 
    mutate(Disturbance = "low")) %>% # Add disturbance column back to sac results
  plot_sac() +
    aes(color=Disturbance) + scale_color_viridis_d()
```
