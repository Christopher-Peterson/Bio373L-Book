...
Notice that `Concentration` is listed as a character vector (`<chr>`). It's probably a good idea to have it as a numeric vector instead. The `names_ptypes` argument lets you specify what type of value each name should be split into (*ptype* stands for "prototype", which is used in `tidyverse` to mean a data type with structure but no values). 

```{r a5_pivot_wider_multi2}

# We're going to create a list that defines the data types of Substrate & Concentration
  # a list() is basically a big box you can put any other kind of data structure into
  # Each iterm in a list is called an elements; elements can be named or not
yeast_name_ptypes = list(
  Substrate = character(),
  Concentration = numeric()
) # This list has 2 elements with names that match the names_to argument of pivot_longer()
...
tidy_yeast = yeast_data %>% 
  pivot_longer(G0.05:U0.3,
    ...
    names_ptypes = yeast_name_ptypes, # We could also have defined the list here
    values_to = "Gene_expression") 
tidy_yeast %>% glimpse()
```
