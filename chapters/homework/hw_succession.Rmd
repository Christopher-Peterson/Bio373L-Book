...
# Homework: Succession {#hw_succession}
...
Information on the data-analysis techniques you'll need to use can be found in Chapter \@ref(r_intro), Chapters \@ref(r_ggplot), Chapters \@ref(r_dplyr), and Chapter \@ref(r_stats) up to and including Section \@ref(r_stats_cat).
...
If it is less that \`r min_r\_version\`\`, you need to re-install R.
...
Download [succession_long_F19.csv]() and save it in the `data` folder.
...
(See section \@ref(r_dplyr_select); section \@ref(r_dplyr_pipe) is also helpful).

## Comparing the relative abundance of canopy and sapling trees in each habitat

You'll need to `filter()` your data (see \@ref(r_dplyr_filter)) so that it only contains a single habitat when doing these comparisons.
...
You should also read up on the pipe `( %>% )`, as it is important for making sequences of steps (\@ref(r_dplyr_pipe)).

```{r hw_filter1, echo = TRUE}
...
For each habitat, you'll need to create a bar plot comparing the relative abundances of canopy and sapling species.
...
You can create this from `data_Q` using a combination of `summarise()` (\@ref(r_dplyr_summarize)) and `group_by()` (\@ref(r_dplyr_group)).
...
The best way to do this is by combining `group_by()` with `mutate()` (\@ref(r_dplyr_mutate)).
...
For this figure, we will sort the data by decreasing canopy abundance, then convert `Species` into a factor (discussed in section \@ref(r_stats_factor)).
...
We'll be making a bar graph using `ggplot`; I'd recommend reading all of Chapter \@ref(r_ggplot), with particular focus on Section \@ref(r_ggplot_discx_discy). 
...
I would recommend changing the `scale_fill_` to a different option so you can customize your colors.
...
Filter `data_Q` so that it only includes species in the top 5, using the `%in%` operator, then use `select()` and `table()` to create a contingency table (\@ref(r_stats_cont_tbl)).
Save the result as `contingency_tbl_Q`.

Finally, you'll need to run a chi-squared test or Fisher's exact test (\@ref(r_stats_chisq)) on `contingency_tbl_Q`.  Use the chi-squared test if all of the cells of the table have a count of 5 or more; otherwise, go with the Fisher's test.  
...
As defined in Section \@ref(lab_succession_density), the point-quarter density estimate is:
...
Finally, let's make a figure showing the distribution of density values at each habitat (any of the Discrete X, Continuous Y plot styles in Section \@ref(r_ggplot_discx_conty) will work).
