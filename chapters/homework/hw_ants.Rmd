...
```{r hw-ants-wide-setup, echo = TRUE}
...
You need to do a grouped `summarize()` on the tidy data so that there's one row per species, with columns Species, N (which is the species-level sum of the already existing N column in `tidy_ant_data`), and whatever your contrast is.
