...
Due to variable weather conditions, phorids may not be abundant enough in the environment to run this experiment.
In previous semesters, we would supplement this with phorids raised in the BFL fire ant lab; however, shifting research priorities and budgetary conditions have resulted in the phorid program's termination.
We will instead attempt to collect wild phorids using a series of baiting stations.
These stations aim to attract phorid flies by concentrating a large number of mostly dead fire ants in a small area.
This is the first time we have attempted this procedure, so if it does not work we will analyze a previous semester's data for this lab.

<!-- ## Modified protocol if phorids are not present -->

<!-- Due to the climate and the state of BFL's phorid fly project, we will not always be able to examine the effects of phorid flies on foraging on fire ants. -->
<!-- We have devised a proxy experiment for these semesters that should disrupt ant foraging in a similar manner. You can skip this section if the phorids are available this semester. -->

<!-- Once a phorid fly attacks a fire ant, the ant releases alarm pheromones ($\alpha$-farnesene). -->
<!-- Therefore, applying alarm pheromones to an experimental arena should simulate the effects of a phorid attack. -->
<!-- In the absence of the purified compound, we can stimulate pheromone production in a variety of ways; here, we will shock them with a modified cattle prod. -->

<!-- In this  experiment, we will have two hot dogs in the each arena.  -->
<!-- One hot dog will be counted for ant activity as in the original design; the other will be shocked with the cattle prod to produce pheromones. -->
<!-- The second hot dog will only be shocked in the treatment group; for the control, the cattle prod will be placed in the same configuration but not used.   -->

<!-- Otherwise, this will follow the same protocol as the original experiment; the time of phorid introduction will be substituted by the time at which the treatment hotdog is shocked. -->
...
To compare leaving rates, you'll need to do an analysis of covariance (ANCOVA), which tests whether the regression between time and proportion of ants differs between the two treatments. Create a figure to accompany this.
...
Does temperature affect foraging behavior?
Does this effect change based on the presence of phorid flies?
